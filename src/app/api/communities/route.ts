...
export async function GET(req: NextRequest) {
  try {
    ...
    if (topic && topic !== "all") {
      conditions.push(eq(communities.topic, topic as any));
    }
    if (search) {
      conditions.push(ilike(communities.name, `%${search}%`));
    }

    const where = conditions.length > 1 ? and(...conditions) : conditions[0];

    const [communitiesList, totalResult] = await Promise.all([
      db
        .select()
        .from(communities)
        .where(where)
      ...
      db.select({ count: count() }).from(communities).where(where),
    ]);
