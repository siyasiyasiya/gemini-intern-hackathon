...
async function fetchCommunities(topic: string, search: string, page: number) {
...
export function CommunityList({ search = "" }: { search?: string }) {
  const [topic, setTopic] = useState("all");
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["communities", topic, search, page],
    queryFn: () => fetchCommunities(topic, search, page),
  });

  const communities: CommunityResponse[] = data?.data || [];
