...
export function RoomHeader({ room, isMember, userRole, onMembershipChange }: RoomHeaderProps) {
  ...
  async function handleJoinLeave() {
    ...
    try {
      ...
      const res = await fetch(`/api/rooms/${room.id}/${endpoint}`, { method: "POST" });
  ...
  async function handleInvite() {
    try {
      const res = await fetch(`/api/rooms/${room.id}/invite`, { method: "POST" });
      const json = await res.json();
      if (json.data?.inviteCode) {
        const url = `${window.location.origin}/api/rooms/join/${json.data.inviteCode}`;
