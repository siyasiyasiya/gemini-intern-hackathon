...
export function ConstellationStats({ constellationSlug }: { constellationSlug: string }) {
  const { data: stats } = useQuery({
    queryKey: ["constellation-stats", constellationSlug],
    queryFn: async () => {
      const res = await fetch(`/api/constellations/${constellationSlug}/stats`);
      const json: ApiResponse<ConstellationStatsResponse> = await res.json();
      return json.data;
    },
  });
